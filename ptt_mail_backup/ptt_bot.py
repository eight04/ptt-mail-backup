...
def check_bot(code):
    ...
    if isinstance(code, tuple):
        code,  *args = code
    if code == PTT.ErrorCode.Success:
        if not args:
            return None
        if len(args) == 1:
            return args[0]
        return args
    raise Exception(f"Failed to check bot: {code}")
...
class PTTBot:
    def __init__(self):
        ...
        self.bot = PTT.Library(kickOtherLogin=False)
    ...
    def login(self, user=None, password=None):
        ...
        check_bot(self.bot.login(user, password))
        
    def logout(self):
        check_bot(self.bot.logout())
        # self.unt("請輸入代號")
    ...
    def get_last_index(self):
        return check_bot(self.bot.getNewestIndex())
    ...
    def get_article(self, index):
        result = check_bot(self.bot.getMail(index))
        if result:
